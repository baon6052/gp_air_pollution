...
def get_datasets(
    ...
    y_low_fidelity = np.array(
        [
            y_value + (noise_multiplier * random_noise)
...
@click.option("--noise_multiplier", default=0.5)
def main(
    climate_variables,
...
def run_multifidelity_analysis(
    ...
    X_train, Y_train, test_data = get_datasets(
        climate_variables,
        noise_multiplier=noise_multiplier,
        num_values_high_fidelity=num_high_fidelity_samples,
        num_values_low_fidelity=num_low_fidelity_samples,
    )
